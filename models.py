...
class RegistroMedico(BaseModel):
    ...
    @validator('ritmo_cardiaco', 'frecuencia_respiratoria', 'presion_sistolica', 'presion_diastolica', 'pasos')
...
for _, row in df_registros.iterrows():
    try:
        ...
        print(f"Error en registro paciente ID {row['paciente_id']} timestamp {row['timestamp']}: {e}")
